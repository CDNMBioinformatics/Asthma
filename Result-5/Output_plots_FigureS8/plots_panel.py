...
df = pd.read_csv("../Output/severe.txt",sep ='\t')
df = df.T
...
columns  = ['Treg','Stimulant_I', 'Stimulant_II', 'Stimulant_III',
       ...
       'Th0', 'Th2', 'Th17', 'Th1', 
        ...
        'Leukotriene', 'Histamine', 'PGD2',  'Epithelial_Cells', 'Eotaxin',  
...
axes = data_sub.plot(subplots=True, layout=(7,6), figsize=(10, 10), 
...
for i in range(7):
    for j in range(6):
          ...
          axes[i,j].set_xticklabels(x_ticks,rotation=45, fontsize=8)
          axes[i,j].set_yticklabels(y_ticks,rotation=0, fontsize=8)
          k+=1

print(plt.style.available)
plt.suptitle('SEVERE', fontsize=24)
plt.tight_layout()
plt.savefig('lineplot_severe.pdf',bbox_inches='tight')
